...
import 'package:base_app/domain/auth/_commons/user_create/user_create.dart';
import 'package:base_app/domain/auth/failure/auth_failure.dart';
import 'package:base_app/domain/auth/value_objects/email_address.dart';
import 'package:base_app/domain/auth/value_objects/password.dart';
import 'package:base_app/presentation/_commons/constance/message_string.dart';
  ...
  void _submit(Submit event, Emitter<RegisterState> emit) async {
    if (state.userCreate == null || !state.userCreate!.isvalid) {
      emit(state.copyWith(
          ...
          authFailureOrSuccessOption: some(
              Left(AuthFailure.fieldError(MessagesStrings.allFieldRequired)))));
      emit(state.copyWith(
          showErrorMessages: false, authFailureOrSuccessOption: none()));
      return;
    }
    ...
    if (!event.conditionCheck) {
      emit(state.copyWith(
          fieldKey: '',
          showErrorMessages: true,
          authFailureOrSuccessOption: some(Left(AuthFailure.fieldError(
              MessagesStrings.youMustAcceptCondition)))));
      emit(state.copyWith(
          showErrorMessages: false, authFailureOrSuccessOption: none()));
      return;
    }
    ...
    failedOrSuccessResponse.fold((l) {
      emit(state.copyWith(
          isSubmitting: false,
          showErrorMessages: true,
