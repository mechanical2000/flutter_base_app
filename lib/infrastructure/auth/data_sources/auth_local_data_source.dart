...
import 'package:base_app/domain/auth/_commons/user/user.dart';
...
class AuthLocalDataSource implements IAuthLocalDataSource {
  ...
  AuthLocalDataSource(
      {required this.sharedPreferences, required this.userSession});

  @override
  Future<bool> cacheUser(AuthResponseDto authResponseDto) async {
    return await sharedPreferences.setString(
        user, jsonEncode(authResponseDto.toJson()));
  }
  ...
  Future<bool> updateUser(User bAUser) async {
    ...
    AuthResponseDto authResponseDto =
        AuthResponseDto(user: UserDto.fromDomain(bAUser), token: userDto.token);
    return await sharedPreferences.setString(
        user, jsonEncode(authResponseDto.toJson()));
  }
}
